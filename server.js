...
async function updatedatabase() {
  await ShortUrl.deleteMany({
    ...
    europeclicks: 0,
    africaclicks: 0,
    northamericaclicks: 0,
    southamericaclicks: 0,
    australiaclicks: 0,
    antarcticaclicks: 0,
  });
}
...
app.post('/shortUrls', async function(req, res, next) {
  try {
    await ShortUrl.create({
      full: req.body.fullUrl,
      short: req.body.shortUrl,
    ...
    err.message="The short URL is already taken. Please go back to homepage and try with another URL."
    next(err);
  }
});
...
function fetchlocation() {
  ...
  https
    .get("https://api.db-ip.com/v2/free/self", (res) => {
      let data = "";
